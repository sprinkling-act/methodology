# The AI Act gate framework

Six sequential regulatory gates that place an AI system under the EU AI Act (Regulation (EU) 2024/1689).

**Where this sits.** This is the regulatory layer, and it comes *after* the analysis, not before it. The work Sprinkling Act sells reconstructs how one workflow actually runs today; see [`README.md`](README.md). Most work sold as governance assumes the reader already knows that. Once a workflow is reconstructed, this framework is what names which obligations its AI components fall under. Used the other way round, it reads a system as it was designed rather than as it runs.
...
| Band | Classification | Note |
|---|---|---|
| Safety component | High risk, Art. 6(1) | Full Art. 9-15 obligations. Third-party conformity assessment required. |
| Annex III | High risk, Art. 6(2) | Full Art. 9-15 obligations. Registration in the EU database required. The Art. 6(3) exception may apply, with documented justification. |
| GPAI / transparency | Limited risk | Art. 50 or Art. 53 obligations. Disclosure requirements apply. |
| Neither | Minimal risk | No mandatory obligation beyond Art. 4 AI literacy. Voluntary code of conduct available. |

Earlier versions of this document attached a 0-100 score to each band. That score belonged to the scored diagnostic Sprinkling Act ran until mid-2026, a product that no longer exists, and it is not reintroduced here: a number of that kind reads as a measurement when it is a classification. The gate that triggered, and the articles it carries, are the finding.
...
Sprinkling Act does not resolve ambiguity. It **flags ambiguity explicitly** and refers edge cases to qualified counsel.
...
Moved to [`CHANGELOG.md`](CHANGELOG.md), which now covers the analysis method and the gate framework in one place.
...
- Not affiliated with any AI vendor, regulator, or certification body. Sprinkling Act is a independent analysis firm registered in Belgium (BCE BE 1034.962.482), not a notified body within the meaning of Regulation (EU) 2024/1689.
